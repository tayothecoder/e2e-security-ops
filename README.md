...
This started as coursework for an MSc in Cybersecurity and evolved into a proper lab environment. Every component talks to the others: the attack scripts generate traffic that Suricata catches, the alerts feed into the forensic timeline, and the same network data trains the ML models. Nothing runs in isolation.
...
**Attack Toolkit**
- 9 standalone Python scripts, each targeting a specific vuln class
- Full pentest orchestrator that chains them together
- Automated report generation from attack results

**Digital Forensics**
- Evidence capture with SHA256 integrity hashing and chain of custody tracking
...
| Firewall | iptables |
| Attack Scripts | Python 3, Requests, BeautifulSoup, sqlmap |
...
|
+-- 04-attack/                 9 attack scripts:
|                              recon.py, sqli_manual.py, sqli_sqlmap.py,
|                              xss_scanner.py, file_upload.py,
|                              brute_forcer.py, idor_enum.py,
|                              pentest_orchestrator.py, report_generator.py
|
+-- 05-detection/              Alert correlation engine
|
+-- 06-forensics/              evidence_capture.py, timeline_builder.py,
|                              pcap_analyser.py, ioc_extractor.py
|
+-- 07-ml-detection/           train_model.py, feature_extraction.py,
|                              compare_detection.py, saved models
|
+-- reports/                   Generated output: pentest reports,
|                              forensic timelines, IOC feeds, ML results
|
+-- DEFENCE-GUIDE.md           Technical deep-dive into the defence stack
...
# Run a single attack
python3 recon.py --target http://localhost:5000

# Manual SQL injection with UNION-based extraction
python3 sqli_manual.py --target http://localhost:5000

# Automated SQLi via sqlmap wrapper
python3 sqli_sqlmap.py --target http://localhost:5000

# XSS scanning (reflected + stored)
python3 xss_scanner.py --target http://localhost:5000

# Or run the full pentest chain
python3 pentest_orchestrator.py --target http://localhost:5000 --output ../reports/

# Generate a formatted report
python3 report_generator.py --input ../reports/pentest_results.json
```
...
# Run alert correlation across all log sources
python3 correlate_alerts.py --suricata /var/log/suricata/eve.json \
                            --cowrie ../03-defence/cowrie/logs/cowrie.json \
                            --app ../02-target-app/logs/travelbird.log
```
...
# Capture evidence with integrity verification
python3 evidence_capture.py --source /var/log/suricata/ \
                            --output ../reports/evidence/

# Build a unified timeline
python3 timeline_builder.py --sources ../reports/evidence/ \
...
python3 ioc_extractor.py --input ../reports/timeline.json \
                         --output ../reports/iocs.stix2
```
...
python3 feature_extraction.py --input ../reports/evidence/capture.pcap \
                              --output features.csv

# Train and evaluate models
python3 train_model.py --input features.csv

# Compare ML detection vs Suricata signatures
python3 compare_detection.py --ml-results ml_predictions.csv \
...
| Initial Access | Exploit Public-Facing App | T1190 | `04-attack/sqli_manual.py`, `sqli_sqlmap.py` |
| Initial Access | Valid Accounts | T1078 | `04-attack/brute_forcer.py` |
...
| Credential Access | Brute Force | T1110 | `04-attack/brute_forcer.py` |
| Discovery | File and Directory Discovery | T1083 | `04-attack/idor_enum.py` |
| Collection | Data from Local System | T1005 | `04-attack/sqli_manual.py` (data extraction) |
| Exfiltration | Exfiltration Over Web Service | T1567 | `04-attack/pentest_orchestrator.py` |
...
> Screenshots are generated from a live environment run. Replace placeholders with actual captures.

| View | Description |
|------|-------------|
| ![TravelBird Home](docs/screenshots/travelbird-home.png) | TravelBird application landing page |
| ![Kibana Dashboard](docs/screenshots/kibana-dashboard.png) | Security operations dashboard in Kibana |
| ![Suricata Alerts](docs/screenshots/suricata-alerts.png) | Real-time Suricata alert stream |
| ![Attack Output](docs/screenshots/attack-output.png) | Pentest orchestrator running the full chain |
| ![Forensic Timeline](docs/screenshots/forensic-timeline.png) | Unified forensic timeline from multiple sources |
| ![ML Comparison](docs/screenshots/ml-comparison.png) | ML vs signature-based detection comparison |
| ![ROC Curves](docs/screenshots/roc-curves.png) | ROC curves for both ML models |
| ![Cowrie Captures](docs/screenshots/cowrie-captures.png) | Cowrie honeypot credential captures |
...
If you are using this for coursework, research, or professional development -- good. If you are thinking about pointing these tools at something you should not be touching -- don't.
...
Built as part of an MSc in Cybersecurity. If you have questions, find bugs, or want to extend something, open an issue or reach out.

---

*See [DEFENCE-GUIDE.md](DEFENCE-GUIDE.md) for a detailed technical walkthrough of the defence stack configuration.*
